...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content">
		<?php 
			...
			if(preg_match("/<embed\s+(.+?)>/i", $full_content, $matches) || preg_match("/\<video(.*)\<\/video\>/is", $full_content, $matches) || preg_match("/\<object(.*)\<\/object\>/is", $full_content, $matches) || preg_match("/<iframe.*src=\"(.*)\".*><\/iframe>/isU", $full_content, $matches)) {
				// remove it
				$content = str_replace($matches[0], '', $full_content);
				// video HTML
				$video = $matches[0];
			}
			// or URL in the content
			else if(preg_match_all('#\bhttps?://[^\s()<>]+(?:\([\w\d]+\)|([^[:punct:]\s]|/))#', $full_content, $matches)) {
				// remove it
				$content = str_replace($matches[0], '', $full_content);
				// video HTML
				$video = wp_oembed_get($matches[0]);
			}
